...
def get_weight_matrix(base, mode):
    ...
    elif mode == 'droso-propo':
        exci = base * (base > 0)
        exci_sum = exci.sum(axis=0,keepdims=True)
        exci_sum[exci_sum == 0] = 1

        inhi = base * (base < 0)
        inhi_sum = inhi.sum(axis=0)
        inhi_sum[inhi_sum == 0] = 1

        return exci / exci_sum - inhi / inhi_sum
